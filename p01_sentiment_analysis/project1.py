...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    agreement_vector = labels * (feature_matrix @ theta + theta_0)
...
def perceptron_single_step_update(
    ...
    if label * (current_theta @ feature_vector + current_theta_0) <= epsilon:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    theta = theta_total = np.zeros(feature_matrix.shape[1])
    theta_0 = theta_0_total = 0
...
def pegasos_single_step_update(
    ...
    if label * (current_theta @ feature_vector + current_theta_0) <= 1 + epsilon:
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    epsilon = 0.000001
    prediction_vector = feature_matrix @ theta + theta_0

    return np.vectorize(lambda x: 1 if x > epsilon else -1)(prediction_vector)
...
def classifier_accuracy(
    ...
    return accuracy(train_class, train_labels), accuracy(val_class, val_labels)
