...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    # n, k = Y.shape[0], theta.shape[0]
    # gradient = np.stack([
    #     (-1 / temp_parameter / n)
    #     * sum([X[i] * ((Y[i]==j) - probabilities[j][i]) for i in range(n)])
    #     + lambda_factor * theta[j]
    #     for j in range(k)
    # ])
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    """
    assigned_labels_mod3 = get_classification(X, theta, temp_parameter) % 3
